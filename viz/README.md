...
결과(배치 레이아웃 + Gantt + 목적값)를 그림으로 봅니다. shapely 없이도 동작합니다.
...
- Codespaces 는 8501 포트를 자동 포워딩해 브라우저 미리보기 URL을 띄웁니다(devcontainer 설정 포함).
...
## 참고
- 한글이 깨지면 폰트(fonts-noto-cjk)를 설치하세요. devcontainer 는 자동 설치합니다.
- 대회 공식 GUI(PyQt6 alg_tester)는 데스크톱 X 환경이 필요해 브라우저 Codespaces 에서는
  바로 뜨지 않습니다. 브라우저에서는 위 Streamlit 방식을 쓰세요.
